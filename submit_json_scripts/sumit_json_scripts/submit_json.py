...
def make_json(annotations_path, categories):
    count = 0
    P = sorted(os.listdir(annotations_path))
    json_data = {}
    for p in tqdm(P):
        if '.png' in p:
            name = p.split('.')[0]+'.jpg'
            ...
            for category in categories:
                ...
                for l in category_pix:
                    ...
                    #print(name, category, len(segments), len(category_pix))
                if len(category_pix):
...
if __name__ == '__main__':
    ...
    categories = {'car':[0,0,255],'signal':[255,255,0], 'pedestrian':[255,0,0],'lane':[69,47,142]}
